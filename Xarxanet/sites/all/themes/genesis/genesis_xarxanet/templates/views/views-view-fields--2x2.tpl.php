  ...
  <div class="event-data">
    <div class="esvent-data">
      ...
      <?php if ($fields['field_date_event_value']->raw!= $fields['field_date_event_value2']->raw):?>
		  ...
		  //print date("d/m/Y \a \l\e\s H:i",  strtotime($fields['field_date_event_value']->raw)+7200);
		  ?></div>
          </div>
          <div class="esvent-data">
			...
			if ($date_event_fecha != "" && $date_event_hora !="")  { print  ($date_event_fecha." a les ".$date_event_hora); }
			//print date("d/m/Y \a \l\e\s H:i",  strtotime($fields['field_date_event_value2']->raw)+7200);
			?></div>
          </div>
    <?else:?>
        <?php 
          ...
          $date_event_hora = substr($fields['field_date_event_value']->content, -5);
          if ($date_event_fecha != "" && $date_event_hora !="")  { print  ($date_event_fecha." a les ".$date_event_hora); }
		//print date("d/m/Y",  strtotime($fields['field_date_event_value']->raw));
		?></div>
          </div>
    <?php endif;?>
    </div>
