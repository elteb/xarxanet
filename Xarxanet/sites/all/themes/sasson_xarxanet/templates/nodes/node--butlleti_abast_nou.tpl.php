...
foreach ($wrapper->field_abast_destacades as $destacada){
	...
	if ($num_destacades != 0 && (($noticia) || ($titular))){
		if ($img){
			...
			$alt = $img[0]['alt'];
		}elseif (!empty($titular->field_agenda_imatge['und'])){
			$imatge = image_style_url('abast-gran',$titular->field_agenda_imatge['und'][0]['uri']);
			$alt = $noticia->field_agenda_imatge['und'][0]['alt'];
		}else{
			$imatge = image_style_url('abast-gran',$titular->field_imatges['und'][0]['uri']);
...
$noticies = array();
foreach ($wrapper->field_abast_destacades as $noticia){
	...
	$imatge = $noticia->field_abast_crop1->value();
