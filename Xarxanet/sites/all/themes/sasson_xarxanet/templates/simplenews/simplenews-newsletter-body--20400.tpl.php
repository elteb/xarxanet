...
<table class="butlleti" style="font-family: Verdana; font-size: 11px;" width="970px" style="margin:auto" cellspacing="8px">
	...
	<tr><td colspan="2">		
		...
		<a style="font-weight:bold; color:#7b1b1c; text-decoration:none" href="<?php echo $pathroot.'/'.$node->path; ?>">View the web version</a></p>	
			...
			<td style="background-color: #7b1b1c" width="100%">
				<table class="butlleti" style="width:100%">
					<tr><td style="font-family:Arial; font-weight:bold; color:white; vertical-align:top;">
						<p style="font-size:24px; margin:5px; color:white;"><?php echo $node->field_abast_titol_butlleti[0]['value']; ?></p>
						<?php if (!empty($node->field_abast_titol_monografic[0]['value'])) {
							echo '<p style="font-size:18px; margin:5px; color:white;">'.$node->field_abast_titol_monografic[0]['value'].'</p>';
						}?>
		...
		<td colspan="2">
			...
			<table class="butlleti" style="width:100%; font-size:11px;">
				<tr><td style="font-weight:bold">
					<?php
					$created = time(); 
					echo date('D, d F Y', $created);
					?>
				</td>
				<td style="text-align: right; font-weight:bold">
					<a style="color:#4D4D4D; text-decoration:none" href="http://www.xarxanet.org/especial/barcelona-ecv">Previous newsletters</a>
				</td></tr>
			</table>
		</td>
	</tr>
	...
	<?php if (!empty($noticies)){?>
		<tr><td colspan="2"><a name="noticies"></a><h1 style="font-family:Arial; font-size:17px; font-weight:bold; color:#818181; margin: 10px 0 5px;">:: Opinion</h1></td></tr>
		<?php
		$curta = true;
	...
	<!-- BANNERS -->
		...
		<?php
			foreach ($node->field_abast_banner_fixe as $banner){
				...
				echo '<img src="'.$banners_fixes[$banner['value']][0].'" alt="'.$banner['value'].'" style="margin: 10px 0; border:0 none;" vspace="10"/></a>';
			}
		?>
		<!-- BANNERS NOUS -->
		<?php
			foreach ($node->field_abast_banner_nou as $banner){
				if(isset($banner['filepath'])){
					...
					echo '<img src="'.$pathroot.'/'.$banner['filepath'].'" alt="'.$banner['data']['alt'].'" style="margin: 10px 0; border:0 none;" vspace="10"/></a>';
				}
			} 
		?>			
