...
class panels_renderer_editor extends panels_renderer_standard {
	...
	/**
	 * An array of AJAX commands to return.
	 * If populated it will automatically
	...
	function get_pane_links($pane, $content_type) {
		...
		if (user_access ( 'administer pane access' )) {
			...
			$links [] = array (
					'title' => '<span class="dropdown-header">' . t ( 'Visibility rules' ) . '</span>' . theme_links ( array (
							'links' => $visibility_links,
							'attributes' => array (),
							'heading' => array () 
					) ),
					'html' => TRUE,
		...
		if (user_access ( 'use panels locks' )) {
			...
			$links [] = array (
					'title' => '<span class="dropdown-header">' . t ( 'Locking' ) . '</span>' . theme_links ( array (
							'links' => $lock_links,
							'attributes' => array (),
							'heading' => array () 
					) ),
	 ...
	 *
	 * @return array An array of categories. Each entry in the array will also be an array
	...
	function ajax_cache_settings($pid = 0) {
		...
		$form_state = array (
				...
				'ajax' => TRUE,
				'title' => $title,
				'url' => url ( $this->get_url ( 'cache-settings', $pid ), array (
						'absolute' => TRUE 
				) ) 
		);
	 ...
	 * @param $pid The
	 *        	pane id, if a pane. The region id, if a region.
	 */
	...
	function ajax_style_settings($type, $pid = '') {
		...
		$form_state = array (
				...
				'ajax' => TRUE,
				'title' => $title,
				'url' => url ( $this->get_url ( 'style-settings', $type, $pid ), array (
						'absolute' => TRUE 
				) ),
				'renderer' => &$this 
		);
