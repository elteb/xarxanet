 ...
 * Fetchers, parsers and processors are
 * declared to Feeds as plugins.
 *
 * @see feeds_feeds_plugins()
 * @see FeedsFetcher
 * @see FeedsParser
 * @see FeedsProcessor @defgroup pluginapi Plugin API
 *      @{
 */

/**
 * Example of a CTools plugin hook that needs to be implemented to make
 * hook_feeds_plugins() discoverable by CTools and Feeds.
 * The hook specifies
 ...
 * plugins to Feeds.
 * For an example look at feeds_feeds_plugin(). For exposing
...
function hook_feeds_before_update(FeedsSource $source, $item, $entity_id) {
	if ($entity_id) {
		...
		db_update ( 'foo_bar' )->fields ( array (
				'entity_type' => $processor->entityType (),
				'entity_id' => $entity_id,
				'last_seen' => REQUEST_TIME 
		) )->condition ( 'entity_type', $processor->entityType () )->condition ( 'entity_id', $entity_id )->execute ();
	}
}
 ...
 *        	FeedsSource object that describes the source that is being imported.
 * @param $entity The
 *        	entity object that has just been saved.
 * @param array $item
 *        	The parser result for this entity.
 * @param int|null $entity_id
 *        	The id of the current item which is going to be updated. If this is a new
 ...
 *
 * @param $source The
 *        	FeedsSource object being imported.
 * @param $result The
 *        	FeedsParserResult object being mapped from.
 * @param $key The
 *        	key specified in the $sources array in
 *        	hook_feeds_parser_sources_alter().
 *        	
 * @return The value to be extracted from the source.
 *        
 ...
 * Use this hook to add additional target
 ...
 *        	entity type of the target, for instance a 'node' entity.
 * @param $bundle_name The
 *        	bundle name for which to alter targets.
 */
 ...
 *
 * @param $source Field
 *        	mapper source settings.
 * @param $entity An
 *        	entity object, for instance a node object.
 * @param $target A
 *        	string identifying the target on the node.
 * @param $value The
 *        	value to populate the target with.
 * @param $mapping Associative
 *        	array of the mapping settings from the per mapping
 *        	configuration form.
 */
 ...
 *
 * @param $mapping Associative
 *        	array of the mapping settings.
 * @param $target Array
 *        	of target settings, as defined by the processor or
 *        	hook_feeds_processor_targets_alter().
 * @param $form The
 *        	whole mapping form.
 * @param $form_state The
 *        	form state of the mapping form.
 *        	
 * @return Returns, as a string that may contain HTML, the summary to display while
 ...
 *
 * @return The per mapping configuration form. Once the form is saved, $mapping will
