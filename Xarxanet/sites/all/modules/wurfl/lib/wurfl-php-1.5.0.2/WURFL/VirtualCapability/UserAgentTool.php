...
class WURFL_VirtualCapability_UserAgentTool {
	...
	protected function assignProperties($device) {
		...
		if (strpos ( $device->ua, 'Android' ) !== false) {
			...
			// Is FB app?
			if ($device->browser->setRegex ( '/^Mozilla\/[45]\.0.+?Android.+?AppleWebKit.+FBAN/', 'FaceBook Android App', $device->os->version ))
				return $device;
			...
			if ($device->browser->setRegex ( '/Chrome\/([0-9]?[0-9])\.?/', 'Chrome Mobile', 1 ))
				...
				// Is UA Fennec?
			if ($device->browser->setRegex ( '/(?:Firefox|Fennec)\/([0-9]?[0-9]\.[0-9]?)/', 'Firefox Mobile', 1 ))
				return $device;
			...
			if ($device->browser->setRegex ( '/Opera Mini\/([0-9]+)?\.[0-9]/', 'Opera Mini', 1 ))
				...
				// Is UA Opera Tablet?
			if ($device->browser->setRegex ( '/Opera Tablet\/.*Version\/([0-9]?[0-9])/', 'Opera Tablet', 1 ))
				return $device;
			...
			if ($device->browser->setRegex ( '/Silk\/([0-9]\.[0-9]).+?Silk\-Accelerated/', 'Amazon Silk Browser', 1 ))
				return $device;
				
				// Is Android Stock Browser?
			if ($device->browser->setRegex ( '/^Mozilla\/[45]\.0.+?Android.+?AppleWeb(?:K|k)it.+?(?:(?:Mobile |)?Safari)/', 'Android', $device->os->version ))
				return $device;
		...
		if (strpos ( $device->ua, 'Silk' ) !== false && $device->browser->setRegex ( '/Silk\/([0-9]\.[0-9]).+?Silk\-Accelerated/', 'Amazon Silk Browser', 1 ) && $device->os->set ( "Android", null ))
			return $device;
		...
		if (strpos ( $device->ua, 'iPhone' ) !== false || strpos ( $device->ua, 'iPad' ) !== false || strpos ( $device->ua, 'iPod' ) !== false) {
			...
			if ($device->browser->setRegex ( '/^Mozilla\/[45]\.0.+?like Mac OS X.+?CriOS\/([0-9]+?)\.[0-9].+?Mobile\/[0-9A-Za-z]+ Safari\/[0-9A-Za-z]+\./', 'Chrome Mobile iOS', 1 ))
				return $device;
				
				// Is UA UC Web Browser?
			if ($device->browser->setRegex ( '/^Mozilla\/[45]\.0.+?like Mac OS X.+?AppleWebKit.+?Mobile\/[0-9A-Za-z]+ Safari\/[0-9A-Za-z]+.*UCBrowser\/?([0-9]+)\./', 'UC Web Browser on iOS', 1 ))
				return $device;
			...
			if ($device->browser->setRegex ( '/^Mozilla\/[45]\.0.+?like Mac OS X.+?AppleWebKit.+?Mobile\/[0-9A-Za-z]+.*FBAN/', 'FaceBook app on iPhone', $device->os->version ))
				...
				// Is UA Mobile iOS Safari?
			if ($device->browser->setRegex ( '/^Mozilla\/[45]\.0.+?like Mac OS X.+?AppleWebKit.+?Mobile\/[0-9A-Za-z]+ Safari\/[0-9A-Za-z]+\./', 'Mobile Safari', $device->os->version ))
				return $device;
		...
		if ($device->os->setContains ( 'Windows CE', 'Windows Mobile' ) && $device->browser->set ( 'IE Mobile' ))
			return $device;
		...
		if (strpos ( $device->ua, 'OviBrowser' ) !== false && $device->browser->setRegex ( '/\bS40OviBrowser\/([0-9]\.[0-9])/', 'S40 Ovi Browser', 1 ) && $device->os->set ( 'Nokia Series 40' ))
			return $device;
			
			// Is Series60?
		if (strpos ( $device->ua, 'Symbian' ) !== false && $device->os->setRegex ( '/(?:SymbianOS\/([0-9]\.[0-9]).+?Series60\/[0-9]\.?[0-9]?)|(?:Series60\/([0-9]\.?[0-9]?) )|(?:S60\/([0-9]\.?[0-9]?))/', 'Symbian S60', 1 )) {
			...
			if ($device->browser->setRegex ( '/Opera Mobi.+Version\/([0-9]?[0-9]\.[0-9]?[0-9])/', 'Opera Mobi', 1 ))
				return $device;
		...
		// Is UA RIM Tablet OS?
		if (strpos ( $device->ua, 'RIM Tablet OS' ) !== false && $device->os->setRegex ( '/RIM Tablet OS ([0-9]\.[0-9]).+?Version\/([0-9]\.[0-9])/', 'RIM Tablet OS', 1 )) {
			$device->browser->set ( 'RIM OS Browser', $device->os->getLastRegexMatch ( 2 ) );
			return $device;
		}
		
		// Is UA Netfront?
		if (strpos ( $device->ua, 'NetFront' ) !== false && $device->browser->setRegex ( '/NetFront\/([0-9]\.[0-9])/', 'NetFront', 1 ))
			return $device;
			
			// Is UA Teleca Obigo
		if ($device->browser->setContains ( 'Obigo', 'Teleca Obigo' ) && $device->browser->setRegex ( '/Obig[a-zA-Z]+?\/(Q[0-9\.ABC]+)/', null, 1 ))
			return $device;
			
			// Is UA Samsung's Bada OS?
		if (strpos ( $device->ua, 'Dolfin' ) !== false && $device->os->setRegex ( '/SAMSUNG.+?\bBada\/([0-9]\.[0-9]);?.+Dolfin\/([0-9]\.[0-9])/', 'Bada', 1 )) {
		...
		if (strpos ( $device->ua, 'Dolfin' ) !== false && $device->browser->setRegex ( '/UP\.(?:Browser|Link)\/([0-9]\.[0-9])/', 'Openwave Browser', 1 ))
			return $device;
		...
		if (strpos ( $device->ua, 'Opera' ) !== false) {
			...
			if ($device->browser->setContains ( 'Opera Mobi', 'Opera Mobile' )) {
				if ($device->browser->setRegex ( '/Opera Mobi.+Version\/([0-9]?[0-9]\.[0-9]?[0-9])/', null, 1 ))
					return $device;
				return $device;
			}
			...
			if ($device->browser->setRegex ( '/Opera Mini\/([0-9]\.[0-9])/', 'Opera Mini', 1 ))
				...
				// Is UA Opera Sync?
			if ($device->browser->setRegex ( '/Browser\/Opera Sync\/SyncClient.+?Version\/([0-9]?[0-9]\.[0-9][0-9]?)/', 'Opera Link Sync', 1 ))
				return $device;
		}
		...
		// MSIE
		if (strpos ( $device->ua, 'MSIE' ) !== false && $device->os->setRegex ( '/^Mozilla\/[0-9]\.0 \(compatible; MSIE ([0-9][0-9]?\.[0-9][0-9]?); ((?:Windows NT [0-9]\.[0-9])|(?:Windows [0-9]\.[0-9])|(?:Windows [0-9]+)|(?:Mac_PowerPC))/', 2 )) {
			$device->browser->set ( 'IE', $device->os->getLastRegexMatch ( 1 ) );
			return $device;
		}
		
		// Yandex Browser
		if (strpos ( $device->ua, 'YaBrowser' ) !== false && $device->os->setRegex ( '/^Mozilla\/[45]\.[0-9] \((?:Macintosh; )?([a-zA-Z0-9\._ ]+)\) AppleWebKit.+YaBrowser\/([0-9]?[0-9]\.[0-9])/', 1 )) {
		...
		// Safari
		if (strpos ( $device->ua, 'Safari' ) !== false && $device->os->setRegex ( '/^Mozilla\/[0-9]\.0 \((?:(?:Windows|Macintosh); (?:U; |WOW64; )?)?([a-zA-Z_ \.0-9]+)(?:;)?.+? Version\/([0-9]+\.[]0-9]+)\.?/', 1 )) {
		...
		// Opera
		if (strpos ( $device->ua, 'Opera' ) !== false && $device->os->setRegex ( '/^Opera\/([0-9]?[0-9]\.[0-9][0-9]?) .+?((?:Windows|Linux|PPC|Intel) [a-zA-Z0-9 _\.\-]+) ?;/', 2 )) {
