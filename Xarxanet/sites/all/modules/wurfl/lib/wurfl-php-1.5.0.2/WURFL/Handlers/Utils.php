...
class WURFL_Handlers_Utils {
	 ...
	 *        	Char index for start of search
	 ...
	 *        	haystack
	 * @param array $needles
	 *        	Array of (string)needles to search for in $userAgent
	...
	public static function isDesktopBrowserHeavyDutyAnalysis($userAgent) {
		...
		if (WURFL_Handlers_Utils::isSmartTV ( $userAgent ))
			return false;
			// Chrome
		if (WURFL_Handlers_Utils::checkIfContains ( $userAgent, 'Chrome' ) && ! WURFL_Handlers_Utils::checkIfContainsAnyOf ( $userAgent, array (
				'Android',
				'Ventana' 
		) ))
			return true;
