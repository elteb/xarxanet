 ...
 * as the completion of a submission or adding validation.
 * Webform's hooks also
 ...
 *
 * @return An array of callbacks that can be used for select list options. This array
 ...
 *
 * @param $component The
 *        	Webform component array for the select component being displayed.
 * @param $flat Boolean
 *        	value indicating whether the returned list needs to be a flat array
 ...
 *        	value indicating whether the included options should be passed
 *        	through the _webform_filter_values() function for token replacement (only)
 *        	needed if your list contains tokens).
 * @param $arguments The
 *        	"options arguments" specified in hook_webform_select_options_info().
 * @return An array of key => value pairs suitable for a select list's #options
 *         FormAPI property.
 */
 ...
 *
 * @param $submissions An
 *        	array of Webform submissions that are being loaded, keyed by the
 ...
 *
 * @param $node The
 *        	Webform node on which this submission was made.
 * @param $submission The
 *        	Webform submission that is about to be saved to the database.
 */
 ...
 *        	Webform node on which this submission was made.
 * @param $submission The
 *        	Webform submission that was just inserted into the database.
 */
function hook_webform_submission_insert($node, $submission) {
	// Insert a record into a 3rd-party module table when a submission is added.
	db_insert ( 'mymodule_table' )->fields ( array (
			'nid' => $node->nid,
			'sid' => $submission->sid,
			'foo' => 'foo_data' 
	) )->execute ();
 ...
 *        	Webform node on which this submission was made.
 * @param $submission The
 *        	Webform submission that was just updated in the database.
 */
function hook_webform_submission_update($node, $submission) {
	...
	db_update ( 'mymodule_table' )->fields ( array (
			'foo' => 'foo_data' 
	) )->condition ( 'nid', $node->nid )->condition ( 'sid', $submission->sid )->execute ();
}
 ...
 *        	Webform node on which this submission was made.
 * @param $submission The
 *        	Webform submission that was just deleted from the database.
 */
function hook_webform_submission_delete($node, $submission) {
	...
	db_delete ( 'mymodule_table' )->condition ( 'nid', $node->nid )->condition ( 'sid', $submission->sid )->execute ();
}
 ...
 *        	Webform node on which this submission was made.
 * @param $submission The
 *        	Webform submission on which the actions may be performed.
 */
 ...
 *
 * @param $renderable The
 *        	Webform submission in a renderable array, similar to FormAPI's
...
function hook_webform_component_delete($component) {
	...
	db_delete ( 'mymodule_table' )->condition ( 'nid', $component ['nid'] )->condition ( 'cid', $component ['cid'] )->execute ();
}

/**
 * Define components to Webform.
 *
 * @return An array of components, keyed by machine name. Required properties are
 ...
 *
 * @param $components A
 *        	list of existing components as defined by hook_webform_component_info().
 *        	
 ...
 *        	Webform submission.
 * @param $op The
 *        	operation to be performed on the submission. Possible values are:
 *        	- "view"
 *        	- "edit"
 *        	- "delete"
 *        	- "list"
 * @param $account A
 *        	user account object.
 * @return TRUE if the current user has access to submission,
 *         or FALSE otherwise.
 */
 ...
 *
 * @param $node The
 *        	Webform node to check access on.
 * @param $account The
 *        	user account to check access on.
 * @return TRUE or FALSE if the user can access the webform results.
 */
 ...
 * @param $value An
 *        	array of information containing the submission result, directly
 *        	correlating to the webform_submitted_data database schema.
 * @return An array of files, each file is an array with following keys:
 ...
 * name of the component type (such as textfield or select).
 * These are not
 ...
 *        	Webform component array.
 * @param $value If
 *        	editing an existing submission or resuming a draft, this will contain
 *        	an array of values to be shown instead of the default in the component
 *        	configuration. This value will always be an array, keyed numerically for
 *        	each value saved in this field.
 * @param $filter Whether
 *        	or not to filter the contents of descriptions and values when
 ...
 * @param $format Either
 *        	'html' or 'text'. Defines the format that the content should be
 *        	returned as. Make sure that returned content is run through check_plain()
 *        	or other filtering functions when returning HTML.
 * @return A renderable element containing at the very least these properties:
 *         - #title
 *         - #weight
 *         - #component
 *         - #format
 ...
 *        	Webform component array.
 * @param $value The
 *        	POST data associated with the user input.
 * @return An array of values to be saved into the database. Note that this should be
 ...
 *
 * @param $component An
 *        	array of information describing the component, directly correlating to
 *        	the webform_component database schema.
 * @param $sids An
 *        	optional array of submission IDs (sid). If supplied, the analysis will
 *        	be limited to these sids.
 * @param $single Boolean
 *        	flag determining if the details about a single component are being
 *        	shown. May be used to provided detailed information about a single
 *        	component's analysis, such as showing "Other" options within a select list.
 * @return An array of data rows, each containing a statistic for this component's
 *         submissions.
 */
function _webform_analysis_component($component, $sids = array(), $single = FALSE) {
	...
	$query = db_select ( 'webform_submitted_data', 'wsd' )->fields ( 'wsd', array (
			'no',
			'data' 
	) )->condition ( 'nid', $component ['nid'] )->condition ( 'cid', $component ['cid'] )->condition ( 'data', '', '<>' )->groupBy ( 'no' )->groupBy ( 'data' );
 ...
 *        	array of information containing the submission result, directly
 *        	correlating to the webform_submitted_data database schema.
 * @return Textual output formatted for human reading.
 */
 ...
 * @param $component A
 *        	Webform component array.
 * @param $export_options An
 *        	array of options that may configure export of this field.
 * @return An array of data to be displayed in the first three rows of a CSV file, not
 ...
 *        	array of information containing the submission result, directly
 *        	correlating to the webform_submitted_data database schema.
 * @return An array of items to be added to the CSV file. Each value within the array
