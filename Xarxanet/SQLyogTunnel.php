...
/* Function gets the names from the database and set the current connection collation to the same */
...
function SetName ( $mysql )
{
    ...
    /* Sdzono code goes here */

	$query = " SHOW  variables like 'character_set_%' ";
	$result = mysql_query ( $query, $mysql );
	$dta = array();
	while($row = mysql_fetch_array($result)){
		$dta[$row['Variable_name']]= $row['Value'];
	}
	
	$db_database = $dta['character_set_database'];
	$db_client = $dta['character_set_client'];
	$db_server = $dta['character_set_server'];

	
	WriteLog( 'DB='.$db_database.'\n' );
	WriteLog( 'Client='.$db_client.'\n' );
	WriteLog( 'Server='.$db_server.'\n' );
