 ...
 * always available.
 *
 * @return An associative array whose keys are hook names and whose values are an
 ...
 *
 * @param $hooks Information
 *        	gathered by module_hook_info() from other modules'
 ...
 * attached).
 *
 * @return An array whose keys are entity type names and whose values identify
 ...
 * altered here.
 *
 * @param $entity_info The
 *        	entity info array, keyed by entity name.
 *        	
 ...
 * @param $type The
 *        	type of entities being loaded (i.e. node, user, comment).
 */
 ...
 *        	entity object.
 * @param $type The
 *        	type of entity being saved (i.e. node, user, comment).
 */
 ...
 *        	entity object.
 * @param $type The
 *        	type of entity being inserted (i.e. node, user, comment).
 */
 ...
 *        	entity object.
 * @param $type The
 *        	type of entity being updated (i.e. node, user, comment).
 */
function hook_entity_update($entity, $type) {
	// Update the entity's entry in a fictional table of all entities.
	$info = entity_get_info ( $type );
	list ( $id ) = entity_extract_ids ( $type, $entity );
	db_update ( 'example_entity' )->fields ( array (
			'updated' => REQUEST_TIME 
	) )->condition ( 'type', $type )->condition ( 'id', $id )->execute ();
}
 ...
 *        	entity object.
 * @param $type The
 *        	type of entity being deleted (i.e. node, user, comment).
 */
 ...
 *        	entity object.
 * @param $type The
 *        	type of entity being rendered (i.e. node, user, comment).
 * @param $view_mode The
 *        	view mode the entity is rendered in.
 * @param $langcode The
 *        	language code used for rendering.
 *        	
 ...
 *
 * @param $build A
 *        	renderable array representing the entity content.
 * @param $type The
 *        	type of entity being rendered (i.e. node, user, comment).
 *        	
 ...
 *
 * @return An associative array. For each item, the key is the path in question, in
 ...
 *
 * @param $paths An
 *        	associative array of administrative paths, as defined by implementations
 *        	of hook_admin_paths().
 *        	
 ...
 *        	type of entities being loaded (i.e. node, user, comment).
 * @param $langcode The
 *        	language to display the entity in.
 */
 ...
 * run in parallel.
 *
 * @return An associative array where the key is the queue name and the value is
 ...
 *
 * @return An associative array describing the element types being defined. The array
 ...
 *
 * @param $type All
 *        	element type defaults as collected by hook_element_info().
 *        	
 ...
 *
 * @param $destination If
 *        	this hook is invoked as part of a drupal_goto() call, then this argument
 ...
 *
 * @param $javascript An
 *        	array of all JavaScript being presented on the page.
 *        	
 * @see drupal_add_js()
 * @see drupal_get_js()
 ...
 *
 * @param $libraries The
 *        	JavaScript/CSS libraries provided by $module. Keyed by internal library
 *        	name and passed by reference.
 * @param $module The
 *        	name of the module that registered the libraries.
 *        	
 * @see hook_library()
 */
 ...
 *
 * @param $commands An
 *        	array of all commands that will be sent to the user.
 *        	
 * @see ajax_render()
 */
 ...
 *
 * @param $page Nested
 *        	array of renderable elements that make up the page.
 *        	
 * @see hook_page_alter()
 ...
 * only.
 *
 * @param $router_item The
 *        	menu router item for $path.
 * @param $path The
 *        	originally passed path, for which $router_item is responsible.
 * @param $original_map The
 *        	path argument map, as contained in $path.
 *        	
 * @see menu_get_item()
 */
 ...
 * altered.
 *
 * @param $items Associative
 *        	array of menu router definitions returned from hook_menu().
 */
 ...
 *
 * @param $item Associative
 *        	array defining a menu link as passed into menu_link_save().
 *        	
 ...
 *
 * @param $item Associative
 *        	array defining a menu link after _menu_link_translate()
 * @param $map Associative
 *        	array containing the menu $map (path parts and/or objects).
 *        	
...
function hook_menu_link_update($link) {
	...
	if ($menu_name != $link ['menu_name']) {
		...
		) )->condition ( 'mlid', $link ['mlid'] )->execute ();
	}
}
...
function hook_menu_link_delete($link) {
	...
	db_delete ( 'menu_example' )->condition ( 'mlid', $link ['mlid'] )->execute ();
}
 ...
 *        	described above.
 * @param $router_item The
 *        	menu system router item of the page.
 * @param $root_path The
 *        	path to the root item for this set of tabs.
 */
 ...
 *
 * @param $active_trail An
 *        	array containing breadcrumb links for the current page.
 * @param $item The
 *        	menu router item of the current page.
 *        	
 ...
 *
 * @param $links An
 *        	associative array containing contextual links for the given $root_path,
 *        	as described above. The array keys are used to build CSS class names for
 *        	contextual links and must therefore be unique for each set of contextual
 *        	links.
 * @param $router_item The
 *        	menu router item belonging to the $root_path being requested.
 * @param $root_path The
 *        	(parent) path that has been requested to build contextual links for.
 ...
 * @param $form_state A
 *        	keyed array containing the current state of the form. The arguments
 *        	that drupal_get_form() was originally called with are available in the
 *        	array $form_state['build_info']['args'].
 * @param $form_id String
 *        	representing the name of the form itself. Typically this is the
 ...
 *
 * @param $form_id The
 *        	unique string identifying the desired form.
 * @param $args An
 *        	array containing the original arguments provided to drupal_get_form()
 ...
 *        	
 * @return An associative array whose keys define form_ids and whose values are an
 ...
 *
 * @return An array with the toolkit name as keys and sub-arrays with these keys:
 ...
 *
 * @param $message An
 *        	array containing the message data. Keys in this array include:
 *        	- 'id':
 ...
 *
 * @param $implementations An
 *        	array keyed by the module's name. The value of each item corresponds
 *        	to a $group, which is usually FALSE, unless the implementation is in a
 *        	file named $module.$group.inc.
 * @param $hook The
 *        	name of the module hook being implemented.
 */
 ...
 * Drupal installation.
 *
 * @return An associative array. Each key is the system name of a theme and each value
...
/**
 * Alter the information parsed from module and theme .
 * info files
 *
 ...
 *
 * @param $info The
 *        	.info file contents, passed by reference so that it can be altered.
 * @param $file Full
 *        	information about the module or theme, including $file->name, and
 *        	$file->filename
 * @param $type Either
 *        	'module' or 'theme', depending on the type of .info file that was
 *        	passed.
 */
 ...
 * For a detailed usage example, see page_example.module.
 *
 * @return An array whose keys are permission names and whose corresponding values
 ...
 * @param $type Whether
 *        	a theme, module, etc. is being processed. This is primarily useful
 ...
 * @param $theme The
 *        	actual name of theme, module, etc. that is being being processed.
 * @param $path The
 *        	directory path of the theme or module, so that it doesn't need to be
 *        	looked up.
 *        	
 * @return array An associative array of theme hook information. The keys on the outer
 ...
 * @endcode
 *
 * @param $theme_registry The
 *        	entire cache of theme registry information, post-processing.
 *        	
 * @see hook_theme()
 ...
 * prevail.
 *
 * @return The machine-readable name of the theme that should be used for the current
 ...
 *
 * @return An array which maps XML-RPC methods to Drupal functions. Each array
 ...
 *
 * @param $methods An
 *        	asssociative array of method callback definitions, as returned from
 ...
 * @param $message An
 *        	array to be filled in. Elements in this array include:
 ...
 * @param $params An
 *        	array of parameters supplied by the caller of drupal_mail().
 */
 ...
 *
 * @param $modules An
 *        	array of the modules that were installed.
 *        	
 * @see module_enable()
 ...
 *
 * @param $modules An
 *        	array of the modules that were enabled.
 *        	
 * @see hook_enable()
 ...
 *
 * @param $modules An
 *        	array of the modules that were disabled.
 *        	
 * @see hook_disable()
 ...
 *
 * @param $modules An
 *        	array of the modules that were uninstalled.
 *        	
 * @see hook_uninstall()
...
function hook_modules_uninstalled($modules) {
	foreach ( $modules as $module ) {
		db_delete ( 'mymodule_table' )->condition ( 'module', $module )->execute ();
	}
 ...
 *
 * @return A nested array, keyed first by scheme name ("public" for "public://"),
 ...
 *
 * @param $files An
 *        	array of file objects, indexed by fid.
 *        	
 ...
 *
 * @param $file The
 *        	file object being validated.
 * @return An array of error messages. If there are no problems with the file return
 *         an empty array.
 *        
 * @see file_validate()
 */
 ...
 *
 * @param $file The
 *        	file that has just been updated.
 *        	
 * @see file_save()
 */
 ...
 *
 * @param $file The
 *        	newly copied file object.
 * @param $source The
 *        	original file before the copy.
 *        	
 * @see file_copy()
 */
 ...
 *
 * @param $file The
 *        	updated file object after the move.
 * @param $source The
 *        	original file object before the move.
 *        	
 * @see file_move()
 */
 ...
 * @param $uri The
 *        	URI of the file.
 * @return If the user does not have permission to access the file, return -1. If the
 ...
 *
 * @param $uri The
 *        	URI to a file for which we need an external URL, or the path to a
 *        	shipped file.
 */
 ...
 *        	status report page.
 *        	
 * @return An associative array where the keys are arbitrary but must be unique (it
 ...
 *
 * @return array A schema definition structure array. For each element of the
 *         array, the key is a table name and the value is a table structure
 *         definition.
 *        
 * @see hook_schema_alter() @ingroup schemaapi
 */
 ...
 *
 * @param $schema Nested
 *        	array describing the schemas for all modules.
 *        	
 *        	@ingroup schemaapi
 */
 ...
 *
 * @param $query A
 *        	Query object describing the composite parts of a SQL query.
 *        	
 ...
 *      
 * @param $sandbox Stores
 *        	information for multipass updates. See above for more information.
 *        	
 ...
 *        
 * @return Optionally, update hooks may return a translated string that will be
...
function hook_update_N(&$sandbox) {
	...
	$users = db_select ( 'users', 'u' )->fields ( 'u', array (
			'uid',
			'name' 
	) )->condition ( 'uid', $sandbox ['current_uid'], '>' )->range ( 0, 3 )->orderBy ( 'uid', 'ASC' )->execute ();
 ...
 *
 * @return A multidimensional array containing information about the module update
 ...
 * the same directory as mymodule.module.
 *
 * @return An integer, corresponding to hook_update_N() which has been removed from
 *         mymodule.install.
 *        
 * @see hook_update_N()
 */
 ...
 *
 * @param $files List
 *        	of files to be parsed by the registry. The list will contain
 ...
 *        	@endcode
 * @param $modules An
 *        	array containing all module information stored in the {system} table.
 ...
 *        	
 * @return array A keyed array of tasks the profile will perform during the final stage of
 ...
 *
 * @param $path A
 *        	Drupal path or a full URL.
 * @param $options An
 *        	associative array of additional URL options to pass to url().
 * @param $http_response_code The
 *        	HTTP status code to use for the redirection. See drupal_goto() for more
 *        	information.
 */
 ...
 *
 * @param $head_elements An
 *        	array of renderable elements. Generally the values of the #attributes
 ...
 *
 * @param $tasks An
 *        	array of all available installation tasks, including those provided by
 ...
 *        	is selected.
 * @param $install_state An
 *        	array of information about the current installation state.
 */
 ...
 *
 * @return An associative array of action descriptions. The keys of the array
 ...
 *
 * @param $info Archiver
 *        	information to alter (return values from hook_archiver_info()).
 */
 ...
 *
 * @return An array of date types where the keys are the machine-readable names and
 ...
 *
 * @param $types A
 *        	list of date types. Each date type is keyed by the machine-readable name
 ...
 *
 * @return A list of date formats to offer as choices in the administrative
 ...
 *
 * @param $theme_groups An
 *        	associative array containing groups of themes.
 *        	
 ...
 *
 * @param $path The
 *        	path being constructed, which, if a path alias, has been resolved to a
 *        	Drupal path by the database, and which also may have been altered by other
 *        	modules before this one.
 * @param $original_path The
 *        	original path, before being checked for path aliases or altered by any
 *        	modules.
 * @param $path_language The
 ...
 *
 * @param $path The
 *        	outbound path to alter, not adjusted for path aliases yet. It won't be
 ...
 * @param $options A
 *        	set of URL options for the URL so elements such as a fragment or a query
 *        	string can be added to the URL.
 * @param $original_path The
 *        	original path, before being altered by any modules.
 *        	
 * @see url()
 */
 ...
 *
 * @param $name The
 *        	string that format_username() will return.
 *        	
 * @param $account The
 *        	account object passed to format_username().
 *        	
 * @see format_username()
 */
 ...
 *
 * @param $type The
 *        	machine-readable name of the type (group) of token being replaced, such
 *        	as 'node', 'user', or another type defined by a hook_token_info()
 *        	implementation.
 * @param $tokens An
 *        	array of tokens to be replaced. The keys are the machine-readable token
 ...
 *        	
 * @return An associative array of replacement values, keyed by the raw [type:token]
 ...
 *
 * @param $replacements An
 *        	associative array of replacements returned by hook_tokens().
 * @param $context The
 *        	context in which hook_tokens() was called. An associative array with
 ...
 *
 * @return An associative array of available tokens and token types. The outer array
 *         has two components:
 ...
 *
 * @param $data The
 *        	associative array of token definitions from hook_token_info().
 *        	
 * @see hook_token_info()
 */
 ...
 *
 * @param $batch The
 *        	associative array of batch information. See batch_set() for details on
 *        	what this could contain.
 *        	
 * @see batch_set()
 * @see batch_process() @ingroup batch
 */
 ...
 * install a new theme.
 *
 * @return An associative array of information about the updater(s) being provided.
 ...
 *
 * @param $countries The
 *        	associative array of countries keyed by ISO 3166-1 country code.
 *        	
 ...
 *
 * @param $menu_site_status Supported
 ...
 * @param $path Contains
 *        	the system path that is going to be loaded. This is read only,
 ...
 * the file operations.
 *
 * @return array Nested array of information about FileTransfer classes. Each key is a
 ...
 *
 * @param $entity The
 *        	entity for which to generate the label.
 * @param $entity_type The
 *        	entity type; e.g., 'node' or 'user'.
 *        	
 * @return An unsanitized string with the label of the entity.
 *        
 * @see entity_label()
 */
 ...
 *
 * @param $entity The
 *        	entity for which to return the language.
 * @param $entity_type The
 *        	entity type; e.g., 'node' or 'user'.
 *        	
 * @return The language code for the language of the entity.
 *        
 * @see entity_language()
 */
