 ...
 *
 * @param $users An
 *        	array of user objects, indexed by uid.
 *        	
 ...
 * @param $account The
 *        	user object on which the operation is being performed.
 * @param $method The
	...
	switch ($method) {
		case 'user_cancel_block_unpublish' :
			...
			$nodes = db_select ( 'node', 'n' )->fields ( 'n', array (
					'nid' 
			) )->condition ( 'uid', $account->uid )->execute ()->fetchCol ();
 ...
 *
 * @return An array of associative arrays. Each inner array has elements:
 ...
 *
 * @param $edit The
 *        	array of form values submitted by the user. Assign values to this
 *        	array to save changes in the database.
 * @param $account The
 *        	user object on which the operation is performed. Values assigned in
 *        	this object will not be saved in the database.
 * @param $category The
 *        	active category of user information being edited.
 *        	
 ...
 *        	array of form values submitted by the user.
 * @param $account The
 *        	user object on which the operation is performed.
 * @param $category The
 *        	active category of user information being edited.
 *        	
 * @see hook_user_presave()
 ...
 *        	array of form values submitted by the user.
 * @param $account The
 *        	user object on which the operation was just performed.
 */
...
function hook_user_logout($account) {
	db_insert ( 'logouts' )->fields ( array (
			'uid' => $account->uid,
			'time' => time () 
	) )->execute ();
}
 ...
 *
 * @param $build A
 *        	renderable array representing the user.
 *        	
 * @see user_view()
 ...
 *
 * @param $role The
 *        	$role object being deleted.
 */
function hook_user_role_delete($role) {
	// Delete existing instances of the deleted role.
	db_delete ( 'my_module_table' )->condition ( 'rid', $role->rid )->execute ();
}
