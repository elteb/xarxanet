 ...
 *
 * @return A nested array of 'pseudo-field' elements. Each list is nested within the
 ...
 *
 * @param $info The
 *        	associative array of 'pseudo-field' components.
 *        	
 ...
 *      
 * @return An array whose keys are field type names and whose values are arrays
 ...
 *
 * @param $info Array
 *        	of information on field types exposed by hook_field_info()
 *        	implementations.
 */
 ...
 *        	of entities being loaded, keyed by entity ID.
 * @param $field The
 *        	field structure for the operation.
 * @param $instances Array
 *        	of instance structures for $field for each entity, keyed by entity
 *        	ID.
 * @param $langcode The
 *        	language code associated with $items.
 * @param $items Array
 *        	of field values already loaded for the entities, keyed by entity ID.
 *        	Store your changes in this parameter (passed by reference).
 * @param $age FIELD_LOAD_CURRENT
 *        	to load the most recent revision for all fields, or
 ...
 *        	type of $entity.
 * @param $entities Array
 *        	of entities being displayed, keyed by entity ID.
 * @param $field The
 *        	field structure for the operation.
 * @param $instances Array
 *        	of instance structures for $field for each entity, keyed by entity
 *        	ID.
 * @param $langcode The
 *        	language associated to $items.
 * @param $items $entity->{$field['field_name']},
 ...
 *        	or an empty array if unset.
 * @param $errors The
 *        	array of errors (keyed by field name, language code, and delta) that
 ...
 *
 * @param $field The
 *        	updated field structure to be saved.
 * @param $prior_field The
 *        	previously-saved field structure.
 * @param $has_data TRUE
 *        	if the field has data in storage currently.
 */
 ...
 * @param $items $entity->{$field['field_name']}[$langcode],
 *        	or an empty array if unset.
 * @param $source_entity The
 *        	source entity from which field values are being copied.
 * @param $source_langcode The
 *        	source language from which field values are being copied.
 */
 ...
 *
 * @param $item An
 *        	item that may or may not be empty.
 * @param $field The
 *        	field to which $item belongs.
 *        	
 * @return TRUE if $field's type considers $item not to contain any data;
 *         FALSE otherwise.
 */
 ...
 *
 * @return An array describing the widget types implemented by the module.
 ...
 *
 * @param $info Array
 *        	of informations on widget types exposed by hook_field_widget_info()
 *        	implementations.
 */
 ...
 *
 * @param $form The
 *        	form structure where widgets are being attached to. This might be a
 *        	full form structure, or a sub-element of a larger form.
 * @param $form_state An
 *        	associative array containing the current state of the form.
 * @param $field The
 *        	field structure.
 * @param $instance The
 *        	field instance.
 ...
 *        	language associated with $items.
 * @param $items Array
 *        	of default values for this field.
 * @param $delta The
 *        	order of this item in the array of subelements (0, 1, 2, etc).
 * @param $element A
 ...
 *        	above.
 *        	
 * @return The form elements for a single widget for this field.
 *        
 ...
 *
 * @param $element The
 *        	field widget form element as constructed by hook_field_widget_form().
 * @param $form_state An
 *        	associative array containing the current state of the form.
 * @param $context An
 *        	associative array containing the following key-value pairs, matching the
 ...
 *
 * @param $element An
 *        	array containing the form element for the widget. The error needs to be
 *        	flagged on the right sub-element, according to the widget's internal
 *        	structure.
 * @param $error An
 *        	associative array with the following key-value pairs, as returned by
 *        	hook_field_validate():
 *        	- error: the error code. Complex widgets might need to report different
 *        	errors to different form elements inside the widget.
 *        	- message: the human readable message to be displayed.
 * @param $form The
 *        	form structure where field elements are attached to. This might be a
 *        	full form structure, or a sub-element of a larger form.
 * @param $form_state An
 *        	associative array containing the current state of the form.
 */
 ...
 *
 * @return An array describing the formatter types implemented by the module.
 ...
 *        	ID.
 * @param $langcode The
 *        	language the field values are to be shown in. If no language is
 *        	provided the current language is used.
 * @param $items Array
 *        	of field values for the entities, keyed by entity ID.
 * @param $displays Array
 *        	of display settings to use for each entity, keyed by entity ID.
 *        	
 * @return Changes or additions to field values are done by altering the $items
 ...
 * @param $display The
 *        	display settings to use, as found in the 'display' entry of instance
 *        	definitions. The array notably contains the following keys and values;
 *        	- type: The name of the formatter to use.
 *        	- settings: The array of formatter settings.
 *        	
 * @return A renderable array for the $items, as an array of child elements keyed
 ...
 *
 * @param $entity_type The
 *        	type of $entity; for example, 'node' or 'user'.
 * @param $entity The
 *        	entity for which an edit form is being built.
 * @param $form The
 *        	form structure where field elements are attached to. This might be a
 ...
 *        	associative array containing the current state of the form.
 * @param $langcode The
 *        	language the field values are going to be entered in. If no language
 ...
 *        	type of $entity; for example, 'node' or 'user'.
 * @param $entity The
 *        	entity for which an edit form is being submitted. The incoming form
 ...
 *
 * @param $variables The
 *        	variables array is passed by reference and will be populated with field
 *        	values.
 * @param $context An
 *        	associative array containing:
 ...
 *        	type of $entity; for example, 'node' or 'user'.
 * @param $entity The
 *        	pseudo-entity whose field data is being purged.
 * @param $field The
 *        	(possibly deleted) field whose data is being purged.
 * @param $instance The
 *        	deleted field instance whose data is being purged.
 *        	
 ...
 *
 * @param $output The
 *        	structured content array tree for all of the entity's fields.
 * @param $context An
 *        	associative array containing:
 *        	- entity_type: The type of $entity; for example, 'node' or 'user'.
 *        	- entity: The entity with fields to render.
 ...
 *
 * @param $entity The
 *        	entity being prepared for translation.
 * @param $context An
 *        	associative array containing:
 ...
 * entity.
 *
 * @param $display_language A
 *        	reference to an array of language codes keyed by field name.
 * @param $context An
 *        	associative array containing:
 ...
 *
 * @param $languages A
 *        	reference to an array of language codes to be made available.
 * @param $context An
 *        	associative array containing:
 ...
 *        	type of entity; for example, 'node' or 'user'.
 * @param $bundle The
 *        	bundle that was just deleted.
 * @param $instances An
 *        	array of all instances that existed for the bundle before it was
 *        	deleted.
 */
 ...
 *
 * @return An array describing the storage backends implemented by the module.
 ...
 *
 * @param $info Array
 *        	of informations on storage types exposed by
 ...
 *
 * @param $details An
 *        	array of storage details for fields as exposed by
 ...
 *
 * @param $entity_type The
 *        	type of entity, such as 'node' or 'user'.
 * @param $entities The
 *        	array of entity objects to add fields to, keyed by entity ID.
 * @param $age FIELD_LOAD_CURRENT
 *        	to load the most recent revision for all fields, or
 *        	FIELD_LOAD_REVISION to load the version indicated by each entity.
 * @param $fields An
 *        	array listing the fields to be loaded. The keys of the array are field
 ...
 *
 * @param $entity_type The
 *        	entity type of entity, such as 'node' or 'user'.
 * @param $entity The
 *        	entity on which to operate.
 * @param $op FIELD_STORAGE_UPDATE
 *        	when updating an existing entity,
 *        	FIELD_STORAGE_INSERT when inserting a new entity.
 * @param $fields An
 *        	array listing the fields to be written. The keys and values of the
 *        	array are field IDs.
 */
 ...
 *        	entity on which to operate.
 * @param $fields An
 *        	array listing the fields to delete. The keys and values of the
 *        	array are field IDs.
 */
 ...
 *        	entity type of entity, such as 'node' or 'user'.
 * @param $entity The
 *        	entity on which to operate. The revision to delete is
 ...
 *        	An EntityFieldQuery.
 *        	
 * @return See EntityFieldQuery::execute() for the return values.
 */
 ...
 *        	to load the most recent revision for all fields, or
 *        	FIELD_LOAD_REVISION to load the version indicated by each entity.
 * @param $skip_fields An
 *        	array keyed by field IDs whose data has already been loaded and
 ...
 *        	type of $entity; for example, 'node' or 'user'.
 * @param $entity The
 *        	entity with fields to save.
 * @param $skip_fields An
 *        	array keyed by field IDs whose data has already been written and
 *        	therefore should not be written again. The values associated with these
 *        	keys are not specified.
 * @return Saved field IDs are set set as keys in $skip_fields.
 */
 ...
 *
 * @param $entity_type The
 *        	type of entity; e.g. 'node' or 'user'.
 * @param $bundle The
 *        	bundle name.
 * @param $context The
 *        	context for which the maximum weight is requested. Either 'form', or
 *        	the name of a view mode.
 * @return The maximum weight of the entity's components, or NULL if no components
 *         were found.
 */
 ...
 *
 * @param $display The
 *        	display settings that will be used to display the field values, as
 ...
 *
 * @param $displays An
 *        	array of display settings for the pseudo-fields in the entity, keyed
 ...
 *
 * @param $field The
 *        	field as it will be post-update.
 * @param $prior_field The
 *        	field as it is pre-update.
 * @param $has_data Whether
 *        	any data already exists for this field.
 */
function hook_field_update_forbid($field, $prior_field, $has_data) {
	...
	if ($has_data && count ( $field ['settings'] ['allowed_values'] ) < count ( $prior_field ['settings'] ['allowed_values'] )) {
		...
		$found = $query->fieldCondition ( $prior_field ['field_name'], 'value', $lost_keys )->range ( 0, 1 )->execute ();
		if ($found) {
 ...
 *
 * @param $field The
 *        	field as it is post-update.
 * @param $prior_field The
 *        	field as it was pre-update.
 * @param $has_data Whether
 *        	any data already exists for this field.
 */
 ...
 *
 * @param $instance The
 *        	instance as it is post-update.
 * @param $prior_instance The
 *        	instance as it was pre-update.
 */
 ...
 *
 * @param $field The
 *        	field record just read from the database.
 */
 ...
 *
 * @param $instance The
 *        	instance record just read from the database.
 */
...
function hook_field_purge_field($field) {
	db_delete ( 'my_module_field_info' )->condition ( 'id', $field ['id'] )->execute ();
}
 ...
 *
 * @param $instance The
 *        	instance being purged.
 */
function hook_field_purge_instance($instance) {
	db_delete ( 'my_module_field_instance_info' )->condition ( 'id', $instance ['id'] )->execute ();
}
 ...
 *        	type of $entity; for example, 'node' or 'user'.
 * @param $entity The
 *        	pseudo-entity whose field data to delete.
 * @param $field The
 *        	(possibly deleted) field whose data is being purged.
 * @param $instance The
 *        	deleted field instance whose data is being purged.
 ...
 *
 * @param $op The
 *        	operation to be performed. Possible values: 'edit', 'view'.
 * @param $field The
 *        	field on which the operation is to be performed.
 * @param $entity_type The
 *        	type of $entity; for example, 'node' or 'user'.
 ...
 *        	
 * @return TRUE if the operation is allowed, and FALSE if the operation is denied.
 */
