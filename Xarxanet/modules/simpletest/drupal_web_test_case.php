...
abstract class DrupalTestCase {
	 ...
	 *
	 * @param $test_id Tests
	 *        	with the same id are reported together.
	 */
	 ...
	 *
	 * @param $status Can
	 *        	be 'pass', 'fail', 'exception'.
	 *        	TRUE is a synonym for 'pass', FALSE for 'fail'.
	 * @param $message The
	 *        	message string.
	 * @param $group Which
	 *        	group this assert belongs to.
	 * @param $caller By
	 *        	default, the assert comes from a function whose name starts with
	...
	protected function assert($status, $message = '', $group = 'Other', array $caller = NULL) {
		...
		$connection->insert ( 'simpletest' )->fields ( $assertion )->execute ();
	 ...
	 * the assertion.
	 *
	 * @return Message ID of the stored assertion.
	 *        
	...
	public static function insertAssert($test_id, $test_class, $status, $message = '', $group = 'Other', array $caller = array()) {
		...
		return db_insert ( 'simpletest' )->fields ( $assertion )->execute ();
	}
	 ...
	 *        	ID of the assertion to delete.
	 * @return TRUE if the assertion was deleted, FALSE otherwise.
	 *        
	 * @see DrupalTestCase::insertAssert()
	 */
	public static function deleteAssert($message_id) {
		return ( bool ) db_delete ( 'simpletest' )->condition ( 'message_id', $message_id )->execute ();
	}
	
	/**
	 * Cycles through backtrace until the first non-assertion method is found.
	 *
	 * @return Array representing the true caller.
	 */
	 ...
	 *
	 * @param $value The
	 *        	value on which the assertion is to be done.
	 * @param $message The
	 *        	message to display along with the assertion.
	 * @param $group The
	 *        	type of assertion - examples are "Browser", "PHP".
	...
	protected function assertFalse($value, $message = '', $group = 'Other') {
		return $this->assert ( ! $value, $message ? $message : t ( 'Value @value is FALSE.', array (
				'@value' => var_export ( $value, TRUE ) 
		) ), $group );
	}
	...
	protected function assertNotNull($value, $message = '', $group = 'Other') {
		return $this->assert ( isset ( $value ), $message ? $message : t ( 'Value @value is not NULL.', array (
				'@value' => var_export ( $value, TRUE ) 
		) ), $group );
	}
	...
	protected function verbose($message) {
		if ($id = simpletest_verbose ( $message )) {
			...
			$this->error ( l ( t ( 'Verbose message' ), $url, array (
					'attributes' => array (
							'target' => '_blank' 
					) 
			) ), 'User notice' );
	 ...
	 *        	A list of method names in the test case class to run; e.g.,
	 ...
	 *
	 * @param $parameters An
	 *        	associative array of parameters, keyed by parameter name, and whose
	 *        	values are arrays of parameter values.
	 *        	
	 * @return A list of permutations, which is an array of arrays. Each inner array
...
class DrupalWebTestCase extends DrupalTestCase {
	 ...
	 *
	 * @param $title A
	 *        	node title, usually generated by $this->randomName().
	 * @param $reset (optional)
	 *        	Whether to reset the internal node_load() cache.
	 *        	
	 * @return A node object matching $title.
	 */
	 ...
	 *
	 * @param $settings An
	 *        	associative array of settings to change from the defaults, keys are
	 ...
	 *
	 * @param $settings An
	 *        	array of settings to change from the defaults.
	 *        	Example: 'type' => 'foo'.
	 ...
	 *
	 * @param $type File
	 *        	type, possible values: 'binary', 'html', 'image', 'javascript', 'php', 'sql', 'text'.
	 * @param $size File
	 *        	size in bytes to match. Please check the tests/files folder.
	 ...
	 *        	
	 * @return object false fully loaded user object with pass_raw property, or FALSE if account
	 *         creation fails.
	 */
	 ...
	 *        	of permission names to assign to role.
	 * @param $name (optional)
	 *        	String for the name of the role. Defaults to a random string.
	 * @return Role ID of newly created role, or FALSE if role creation failed.
	 */
	 ...
	 * @return TRUE or FALSE depending on whether the permissions are valid.
	 */
	 ...
	 *
	 * @param $account User
	 *        	object representing the user to log in.
	 *        	
	...
	protected function prepareDatabasePrefix() {
		...
		db_update ( 'simpletest_test_id' )->fields ( array (
				'last_prefix' => $this->databasePrefix 
		) )->condition ( 'test_id', $this->testId )->execute ();
	}
	 ...
	 * Useful after a page request is made
	 ...
	 *
	 * @param $curl_options An
	 *        	associative array of cURL options to set, where the keys are constants
	 ...
	 *        	if this is an initial request, TRUE if this request is the result
	 *        	of a redirect.
	 *        	
	 * @return The content returned from the call to curl_exec().
	 *        
	 * @see curlInitialize()
	 */
	 ...
	 *
	 * @return A SimpleXMLElement or FALSE on failure.
	 */
	protected function parse() {
		if (! $this->elements) {
			...
			if ($htmlDom) {
				$this->pass ( t ( 'Valid HTML found on "@path"', array (
						'@path' => $this->getUrl () 
				) ), t ( 'Browser' ) );
	 ...
	 *
	 * @param $path Drupal
	 *        	path or URL to load into internal browser
	 * @param $options Options
	 *        	to be forwarded to url().
	 * @param $headers An
	 *        	array containing additional HTTP request headers, each formatted as
	 *        	"name: value".
	 ...
	 *        	@endcode
	 * @param $edit Field
	 *        	data in an associative array. Changes the current input fields
	 ...
	 *        	@endcode
	 * @param $submit Value
	 *        	of the submit button whose click is to be emulated. For example,
	 ...
	 *        	array containing additional HTTP request headers, each formatted as
	 *        	"name: value".
	 * @param $form_html_id (optional)
	 *        	HTML ID of the form to be submitted. On some pages
	 *        	there are many identical forms, so just using the value of the submit
	 *        	button is not enough. For example: 'trigger-node-presave-assign-form'.
	 *        	Note that this is not the Drupal $form_id, but rather the HTML ID of the
	 *        	form, which is typically the same thing but with hyphens replacing the
	 *        	underscores.
	 * @param $extra_post (optional)
	 *        	A string of additional data to append to the POST submission.
	...
	protected function drupalPost($path, $edit, $submit, array $options = array(), array $headers = array(), $form_html_id = NULL, $extra_post = NULL) {
		...
		if ($this->parse ()) {
			...
			$this->fail ( t ( 'Found the requested form fields at @path', array (
					'@path' => $path 
			) ) );
		}
	}
	 ...
	 *
	 * @param $path Location
	 *        	of the form containing the Ajax enabled element to test. Can be
	 *        	either a Drupal path or an absolute path or NULL to use the current page.
	 * @param $edit Field
	 *        	data in an associative array. Changes the current input fields
	 *        	(where possible) to the values indicated.
	 * @param $triggering_element The
	 *        	name of the form element that is responsible for triggering the Ajax
	 ...
	 * @param $ajax_path (optional)
	 *        	Override the path set by the Ajax settings of the triggering
	 *        	element. In the absence of both the triggering element's Ajax path and
	 *        	$ajax_path 'system/ajax' will be used.
	 * @param $options (optional)
	 *        	Options to be forwarded to url().
	 * @param $headers (optional)
	 *        	An array containing additional HTTP request headers, each
	 *        	formatted as "name: value". Forwarded to drupalPost().
	 * @param $form_html_id (optional)
	 *        	HTML ID of the form to be submitted, use when there is more
	 ...
	 * @param $ajax_settings (optional)
	 *        	An array of Ajax settings which if specified will be used in
	...
	protected function drupalPostAJAX($path, $edit, $triggering_element, $ajax_path = NULL, array $options = array(), array $headers = array(), $form_html_id = NULL, $ajax_settings = NULL) {
		...
		$return = drupal_json_decode ( $this->drupalPost ( NULL, $edit, array (
				'path' => $ajax_path,
	 ...
	 *
	 * @return Either the new page content or FALSE.
	 */
	 ...
	 *        	array containing additional HTTP request headers, each formatted as
	 *        	"name: value".
	 * @return The retrieved headers, also available as $this->drupalGetContent()
	 */
	 ...
	 * Ensure that the specified fields
	 * exist and attempt to create POST data in the correct manner for the particular
	 * field type.
	 *
	 * @param $post Reference
	 *        	to array of post values.
	 * @param $edit Reference
	 *        	to array of edit values to be checked against the form.
	 * @param $submit Form
	 *        	submit button value.
	 * @param $form Array
	 *        	of form elements.
	 * @return Submit value matches a valid submit input in the form.
	 */
	 ...
	 *
	 * @param $xpath An
	 *        	XPath query, possibly with placeholders in the form ':name'.
	 * @param $args An
	 *        	array of arguments with keys in the form ':name' matching the
	 *        	placeholders in the query. The values may be either strings or numeric
	 *        	values.
	 * @return An XPath query with arguments replaced.
	 */
	 ...
	 *
	 * @param $xpath The
	 *        	xpath string to use in the search.
	 * @return The return value of the xpath search. For details on the xpath string
	 ...
	 *
	 * @param $element The
	 *        	element for which to get the options.
	 * @return Option elements in select.
	 */
	 ...
	 *        	to display.
	 * @param $group The
	 *        	group this message belongs to, defaults to 'Other'.
	 * @return TRUE if the assertion succeeded, FALSE otherwise.
	 */
	protected function assertLink($label, $index = 0, $message = '', $group = 'Other') {
		$links = $this->xpath ( '//a[normalize-space(text())=:label]', array (
				':label' => $label 
		) );
	 ...
	 *
	 * @param $href The
	 *        	full or partial value of the 'href' attribute of the anchor tag.
	 * @param $index Link
	 *        	position counting from zero.
	 * @param $message Message
	 *        	to display.
	 ...
	 * @param $index Link
	 *        	position counting from zero.
	 * @return Page on success, or FALSE on failure.
	 */
	 ...
	 *
	 * @param $path A
	 *        	path from the internal browser content.
	 * @return The $path with $base_url prepended, if necessary.
	 */
	 ...
	 *
	 * @param $all_requests Boolean
	 *        	value specifying whether to return headers from all requests
	 *        	instead of just the last request. Defaults to FALSE.
	 * @return A name/value array if headers from only the last request are requested.
	 ...
	 *
	 * @param $name The
	 *        	name of the header to retrieve. Names are case-insensitive (see RFC
	 *        	2616 section 4.2).
	 * @param $all_requests Boolean
	 *        	value specifying whether to check all requests if the header is
	 *        	not found in the last request. Defaults to FALSE.
	 * @return The HTTP header value or FALSE if not found.
	 */
	 ...
	 *
	 * @param $filter An
	 *        	array containing key/value pairs used to filter the e-mails that are returned.
	 * @return An array containing e-mail messages captured during the current test.
	 */
	 ...
	 * This can be useful when a page has been fetched
	 ...
	 * @param $options (optional)
	 *        	Any additional options to pass for $path to url().
	 * @param $message Message
	 *        	to display.
	 * @param $group The
	 *        	group this message belongs to, defaults to 'Other'.
	 ...
	 * @param $not_exists TRUE
	 *        	if this text should not exist, FALSE if it should.
	 * @return TRUE on pass, FALSE on fail.
	 */
	 ...
	 * @param $group The
	 *        	group this message belongs to.
	 * @param $be_unique TRUE
	 *        	if this text should be found only once, FALSE if it should be found more than once.
	 * @return TRUE on pass, FALSE on fail.
	 */
	 ...
	 *
	 * @param $pattern Perl
	 *        	regex to look for including the regex delimiters.
	 * @param $message Message
	 *        	to display.
	 * @param $group The
	 *        	group this message belongs to.
	 ...
	 *
	 * @param $title The
	 *        	string the title should not be.
	 * @param $message Message
	 *        	to display.
	 * @param $group The
	 *        	group this message belongs to.
	 ...
	 *
	 * @param $callback The
	 *        	name of the theme function to invoke; e.g. 'links' for theme_links().
	 * @param $variables An
	 *        	array of variables to pass to the theme function.
	 * @param $expected The
	 *        	expected themed output string.
	 ...
	 *        	select element.
	 * @return The selected value or FALSE.
	 */
	...
	protected function assertNoFieldByName($name, $value = '', $message = '') {
		return $this->assertNoFieldByXPath ( $this->constructFieldXpath ( 'name', $name ), $value, $message ? $message : t ( 'Did not find field by name @name', array (
				'@name' => $name 
		) ), t ( 'Browser' ) );
	}
	...
	protected function assertNoFieldById($id, $value = '', $message = '') {
		return $this->assertNoFieldByXPath ( $this->constructFieldXpath ( 'id', $id ), $value, $message ? $message : t ( 'Did not find field by id @id', array (
				'@id' => $id 
		) ), t ( 'Browser' ) );
	}
	...
	protected function assertFieldChecked($id, $message = '') {
		$elements = $this->xpath ( '//input[@id=:id]', array (
				':id' => $id 
		) );
		return $this->assertTrue ( isset ( $elements [0] ) && ! empty ( $elements [0] ['checked'] ), $message ? $message : t ( 'Checkbox field @id is checked.', array (
				'@id' => $id 
		) ), t ( 'Browser' ) );
	}
	...
	protected function assertNoFieldChecked($id, $message = '') {
		...
		return $this->assertTrue ( isset ( $elements [0] ) && empty ( $elements [0] ['checked'] ), $message ? $message : t ( 'Checkbox field @id is not checked.', array (
				'@id' => $id 
		) ), t ( 'Browser' ) );
	}
	 ...
	 * @param $group The
	 *        	group this message belongs to.
	 * @param $ids_to_skip An
	 *        	optional array of ids to skip when checking for duplicates. It is
	 ...
	 * @param $value Value
	 *        	of field.
	 * @return XPath for specified values.
	 */
	 ...
	 *
	 * @param $code Response
	 *        	code. For example 200 is a successful page request. For a list
	...
	protected function assertResponse($code, $message = '') {
		...
		return $this->assertTrue ( $match, $message ? $message : t ( 'HTTP response expected !code, actual !curl_code', array (
				'!code' => $code,
				'!curl_code' => $curl_code 
		) ), t ( 'Browser' ) );
	}
	 ...
	 *
	 * @param $name Name
	 *        	of field or message property to assert. Examples: subject, body, id, ...
	 * @param $value Value
	 *        	of the field to assert.
	 * @param $message Message
	 *        	to display.
	 ...
	 *        	of field or message property to assert: subject, body, id, ...
	 * @param $string String
	 *        	to search for.
	 * @param $email_depth Number
	 *        	of emails to search for string, starting with most recent.
	 *        	
	 * @return TRUE on pass, FALSE on fail.
	 */
 ...
 * @param $message The
 *        	verbose message to be stored.
 * @param $original_file_directory The
 *        	original file directory, before it was changed for testing purposes.
 * @param $test_class The
 *        	active test case class.
 *        	
 * @return The ID of the message to be placed in related assertion messages.
 *        
