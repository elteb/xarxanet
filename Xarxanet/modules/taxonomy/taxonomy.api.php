 ...
 *
 * @param $terms An
 *        	array of term objects, indexed by tid.
 */
function hook_taxonomy_term_load($terms) {
	$result = db_select ( 'mytable', 'm' )->fields ( 'm', array (
			'tid',
			'foo' 
	) )->condition ( 'm.tid', array_keys ( $terms ), 'IN' )->execute ();
	foreach ( $result as $record ) {
 ...
 *
 * @param $term The
 *        	term that is being assembled for rendering.
 * @param $view_mode The
 *        	$view_mode parameter from taxonomy_term_view().
 * @param $langcode The
