    ...
    <div class="node-inner">
				...
				if (!empty($node->field_file['und'])): ?>
					<div class="node-files">
						...
						foreach($node->field_file['und'] as $file) {
						    echo '<li>'.l( $file['description'], file_create_url($file['uri'])).'</li>';						
						} 
                    	?>
                    	</ul>
                    </div>
		...
		<?php 		
			if($node->print_html_display || $node->print_mail_display || $node->print_pdf_display) {
				...
				if ($node->print_mail_display) echo '<li class="print_mail">'.l(t('Send to a friend'), 'printmail/'.$node->nid).'</li>';
				if ($node->print_pdf_display) echo '<li class="print_pdf">'.l(t('PDF version'), 'printpdf/'.$node->nid).'</li>';
				echo '</ul></div></div>';
			}
		?>
        </div>
