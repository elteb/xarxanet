 ...
 */
function admin_html_head_alter(&$head_elements) {
...
/*add a admin class to the header to give targetability for css enhancements */
function admin_preprocess_html(&$variables) {
 ...
 */
function admin_process_page(&$vars) {
 ...
 */
function admin_node_add_list($variables) {
 ...
 */
function admin_admin_block_content($variables) {
  $content = $variables['content'];
  $output = '';
  if (!empty($content)) {
 ...
 */
function admin_tablesort_indicator($variables) {
  $style = $variables['style'];
  $theme_path = drupal_get_path('theme', 'admin');
  if ($style == 'asc') {
 ...
 */
function admin_css_alter(&$css) {
  // Use admin's jQuery UI styles instead of the defaults.
  $jquery = array('core', 'dialog', 'tabs', 'theme');
  foreach($jquery as $module) {
    if (isset($css['misc/ui/jquery.ui.' . $module . '.css'])) {
      $css['misc/ui/jquery.ui.' . $module . '.css']['data'] = drupal_get_path('theme', 'admin') . '/styles/jquery.ui.' . $module . '.css';
    }
  }

  // If the media module is installed, Replace it with admin's.
  if (module_exists('media')) {
    $media_path = drupal_get_path('module', 'media') . '/css/media.css';
    if(isset($css[$media_path])) {
      $css[$media_path]['data'] = drupal_get_path('theme', 'admin') . '/styles/media.css';
    }
  }

  // If the responsive vertical tabs module is installed, add admin themeing overrides to it
  if (module_exists('responsive_vertical_tabs')) {
    ...
    if(isset($css[$rvt_theme_path])) {
      $css[$rvt_theme_path]['data'] = drupal_get_path('theme', 'admin') . '/styles/vertical-tabs-theme.css';
    }
  }
}
 ...
 * vertical spacing.
 */
function admin_links__ctools_dropbutton($variables) {
 ...
 */
function admin_views_ui_display_tab_alter(&$build, &$view, $display_id) {
  ...
  foreach ($display->handler->get_option("{$type}s") as $id => $field) {
    ...
    $text = _admin_views_ui_field_link($field, $handler, $relationships, $type, $root_url);
 ...
 */
function _admin_views_ui_field_link($field, $handler, array $relationships, $type, $root_url) {
