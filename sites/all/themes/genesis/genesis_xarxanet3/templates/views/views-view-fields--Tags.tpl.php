...
<div class="<?php print xarxanet_get_class($fields['type']->content); ?>">
  ...
  if($numId == 1){
    ...
    if($rawImatge == "") $rawImatge = theme('imagecache', 'tag-gran', 'no-image.jpg', 'just a test image', 'test image');

  }else if($numId == 2 || $numId == 7){
    $rawImatge = $fields['field_agenda_imatge_fid_1']->content;
    if($rawImatge == "") $rawImatge = theme('imagecache', 'tag-mig', 'no-image.jpg', 'just a test image', 'test image');
  }else{
    $rawImatge = $fields['field_agenda_imatge_fid']->content;
    if($rawImatge == "") $rawImatge = theme('imagecache', 'tag-petit', 'no-image.jpg', 'just a test image', 'test image');
  }
  print $rawImatge;
  ?>
