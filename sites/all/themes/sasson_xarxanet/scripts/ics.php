...
$summary = urldecode($_GET['summary']); //text title of the event
$datestart = urldecode($_GET['datestart']); //the starting date (in seconds since unix epoch)
$dateend = urldecode($_GET['dateend']); //the ending date (in seconds since unix epoch)
$address = urldecode($_GET['adress']); //the event's address
$uri = urldecode($_GET['uri']); //the URL of the event (add http://)
$description = urldecode($_GET['description']); //text description of the event
...
CALSCALE:GREGORIAN
BEGIN:VEVENT
DTEND:<? print dateToCal($dateend)?>

UID:<? print uniqid() ?>

DTSTAMP:<? print dateToCal(time()) ?>

LOCATION:<? print escapeString($address) ?>

DESCRIPTION:<? print escapeString($description) ?>

URL;VALUE=URI:<? print escapeString($uri) ?>

SUMMARY:<? print escapeString($summary) ?>

DTSTART:<? print dateToCal($datestart) ?>
