 ...
 *
 * @param $vote_results An
 *        	alterable array of aggregate vote results.
 * @param $content_type A
 *        	string identifying the type of content being rated. Node, comment,
 *        	aggregator item, etc.
 * @param $content_id The
 *        	key ID of the content being rated.
 *        	
 ...
 *
 * @return An array of records containing 'description', 'content_type', 'base_table',
 ...
 *
 * @param $field A
 *        	Views field object. This can be used to expose formatters only for tags,
 *        	vote values, aggregate functions, etc.
 * @return An array of key-value pairs, in which each key is a callback function and
 ...
 *
 * @param $vote See
 *        	votingapi_add_votes() for the structure of this array, with the
 ...
 *
 * @param $votes An
 *        	array of votes to delete. Minimally, each vote must have the 'vote_id'
 *        	key set.
 * @param $vids A
 *        	list of the 'vote_id' values from $voes.
 */
function hook_votingapi_storage_delete_votes($votes, $vids) {
	mongodb_collection ( 'votingapi_vote' )->delete ( array (
			'vote_id' => array (
 ...
 *
 * @param $criteria A
 *        	keyed array used to build the select query. Keys can contain
 ...
 * @param $limit An
 *        	integer specifying the maximum number of votes to return. 0 means
 *        	unlimited and is the default.
 * @return An array of votes matching the criteria.
 */
